...
export class StrapiService {
  ...
  public static uploadsBaseUrl: string;

  static headers: any;

  constructor() {
    if (SheltifyAccessConfig.strapi.tenant == "") console.warn("SheltifyAccessConfig.strapi.tenant not set - all data will be queried");
  ...
  public get<T>(path: string): Observable<T> {
    const tenantFilter = StrapiService.tenant != "" ? `&filters[createdBy][username][$eq]=${StrapiService.tenant}` : "";
    let url = decodeURIComponent(StrapiService.apiBaseUrl + path);
    url = this.addDraftsInDevMode(url);
    return this.httpClient
      .get(url + tenantFilter, {
